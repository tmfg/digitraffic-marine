...
public class DefaultExceptionHandler {
    ...
    @ExceptionHandler({ObjectNotFoundException.class, ResourceAccessException.class, BadRequestException.class, IllegalArgumentException.class, MethodArgumentTypeMismatchException.class})
    @ResponseBody
    ...
    private ResponseEntity getErrorResponseEntity(final HttpStatus httpStatus, final String errorMsg, final ServletWebRequest request) {
        return new ResponseEntity<>(
            new ErrorResponse(Timestamp.from(ZonedDateTime.now().toInstant()), httpStatus.value(), httpStatus.getReasonPhrase(), errorMsg,
                request.getRequest().getRequestURI()), httpStatus);
    }
